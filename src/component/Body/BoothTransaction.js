...
class BoothTransaction extends Component {
    ...
    download() {
        const {from, to, boothno} = this.state;
        axios.post(`${APP_URL}/download`, {boothno,from,to}).then(res => {
            if(res.data.success) {
                const { saveAsCsv } = useJsonToCsv();
                let fields = {
                    "t_id": "Transaction Id",
                    "e_code": "Employee Code",
                    "b_no": "Booth",
                    "l_no": "Lane",
                    "s_no": "Shift No",
                    "v_type": "Vehicle Type",
                    "v_no": "Vehicle No",
                    "j_type": "Journey Type",
                    "v_st_wt": "Vehicle st wt",
                    "v_wt": "Vehicle wt",
                    "amount": "Amount",
                    "date": "Date",
                }
                saveAsCsv({ data:res.data.data, fields, filename:"transaction" });
            } else {
